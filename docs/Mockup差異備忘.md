...
### 差異 44：B4 手機結果卡精簡 chrome（覆寫 Mockup 勾選／快捷／×／▸）

- **差在哪**：
  1. Mockup 結果卡常駐 checkbox、快捷核准鈕、×、展開三角。
  2. 正式版 **僅手機（<960px）**：隱藏上述 chrome；多選靠 **長按**（B3-P04）、快捷靠 **左滑**、展開靠 **點卡**。
  3. 列表頂一句「長按可多選…」提示（可看過即淡）。
  4. **桌機維持** Mockup／現況勾選與快捷。
- **為什麼**：老闆 2026-07-21 截圖 #6/#7——手機已有長按與滑動，卡面按鈕多餘。
- **誰拍板**：老闆＋X 臨時代理指揮（Grok）2026-07-21。
- **現況**：⏳ **UX-B4-P04 待實作**（指令：`docs/UX-B4-P04工人開場指令-2026-07-21.md`）。
...
- **誰拍板**：老闆＋X 臨時代理指揮（Grok）2026-07-21。
- **現況**：⏳ **UX-B4-P03 待實作**（指令：`docs/UX-B4-P03工人開場指令-2026-07-21.md`）。覆寫 VariantEditor 舊註「never auto full cartesian」。
