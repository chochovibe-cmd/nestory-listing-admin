...
### ⚡ 2026-07-21 UIUX 現況（接手 UI／前台前必讀）

1. **`docs/UIUX本輪改動同步-2026-07-21.md`** — **B4 全包收官真相**（P01～P08 核帳＋commit 表＋勿重做）
2. `docs/UIUX指揮交接-下一任接續.md` — 短交接（5 步）
3. `docs/施工清單.md` — 章節 **UX-B4** 勾選區＋進度筆記
4. Mockup差異 **43／44／45**（自動展開、手機三排、失敗紅字上標題）
5. 歷史：`docs/UIUX本輪改動同步-2026-07-19.md`（B2／PKG；勿當本輪進度）

**現況摘要**：**B4-P01～P08 已全部核帳並 push**。新需求另開包。  
真發 Shopify 需老闆設 `SHOPIFY_PUBLISH_MOCK=false`。  
不要重做 B3 長按／左滑／規格 B 方案本體；不要把輸入縮圖改回 P10 橫滑 nowrap（P08 已覆寫）。
