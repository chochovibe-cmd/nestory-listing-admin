...
搜尋結果經你合理判斷為同款後，把規格、功能、系列背景直接自信地寫進文案，不必加「據網路／可能／應該」等保留語氣。
證據少時：只有下方紅線項目才留白；其餘改用體驗式內容把該段寫完整。不要整段刪除，不要因為資料少就交空洞罐頭句。`;

/** COPY-FIX-2: the only remaining fact red lines (boss 2026-09-07). */
const COPY_FACT_RED_LINES_BLOCK = `【文案紅線｜只有這些沒依據才不准寫】
...
const COPY_REGEN_GUARDRAIL_REMINDER =
  "【放手寫與紅線】體驗、情境、幽默、適合誰不需要證據，請寫滿。" +
  "紅線項目（精確數字、材質、發售年份、官方售價、授權／正版、限定／絕版／停產、庫存／到貨承諾）沒依據不要寫。" +
...
◈ 商品亮點
・列 3 條左右，每條用「重點詞：具體說明」的節奏，講觸感／功能／設計上的具體亮點，讓人看見畫面

◈ 適合誰
・列 2-3 條，具體描繪什麼樣的人、什麼情境會想要這個商品——這段由你自由發揮，不需要證據

◈ 商品資訊（品名、類型、以及有依據的規格；沒有規格時用具體使用畫面補，不要整段刪除）
・有依據就列：品名、類型、材質、尺寸等
・精確數字只寫證據池裡賣家自己標出來的，或同款搜尋結果清楚標出的——
  來源包括：款式／Variant 選項文字、原始標題、詳情圖上印出來被轉錄的規格文字、同款判斷成立的搜尋結果。
  「商品外觀描述」的目測屬性不能拿來當尺寸／重量／容量依據；沒有可靠數字時這一項留白即可
・沒有規格數字也不要整段刪除：保留「◈ 商品資訊」標題，寫品名／類型／看得出的設計特徵，或補一句具體使用情境。紅線項目沒依據才留白，不要捏造
・不要在這裡寫入售價、定價或任何價格數字——價格由 Shopify 商品頁自己的價格欄位顯示
...
數字與事實紅線（與上方【文案紅線】一致）：
- 可以寫：款式選項／原標題／詳情圖轉錄裡「賣家自己標出來」的尺寸／重量等數字——直接自信寫
- 可以寫：網路搜尋結果清楚標出、且你合理判斷為同款的規格數字——直接寫數字，不要標出處，不必加保留語氣
- 沒有可靠來源的精確數字：這一項留白（不要目測估計）
- 不要寫價格
- 【P4 出處標記禁令】描述／spec／賣點／FAQ／meta 等顧客可見欄位一律禁止出現
  「（來源：網路）」「來源：…」「（來源：URL）」或任何出處註記；同款判斷成立的網搜事實可以直接寫，只是不標出處
...
把其中的規格、功能、系列背景當作可用事實正面使用，直接自信地寫進文案。規則：
