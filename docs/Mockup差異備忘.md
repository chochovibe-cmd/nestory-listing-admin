...
### 差異 52：潮巢導購在 CC-6 五段上補強具體度與看圖畫面感（不改回三段）

- **差在哪**：
  1. 正文仍是商品介紹／收藏亮點／適合誰／商品資訊／購買提醒，**不是** Mockup／C1.1 的「導購小標」三段，也不是 ◈ 四段。
  2. 潮巢專屬 prompt 要求至少一句角色／IP 粉絲才會點頭；選品理由／賣點／FAQ 要「換掉商品名就不成立」。其他 6 語氣規則不跟改。
  3. 看圖外觀描述可寫畫面感、總長約 300–500 字；尺寸數字仍只有圖上有印才抄。模型仍 gpt-4o-mini。
- **為什麼**：Owner 給的 2026-09-24 實作清單對的是舊共用 prompt 行號。現役已是 CC-6 獨立五段；改回三段會推翻差異 49／50。CC-6 不要空泛句禁詞池，所以用正面「換掉名字就不成立」而不是長禁詞。
- **誰拍板**：Owner 2026-09-24 丟清單並說推 Preview；實作時依 CC-6／差異 50 映射，不照清單把結構改回三段。
- **現況**：✅ `0410016` 已 push `agent/chaochao-tone-on-live`。E 知識包、F 角色搜尋沒做。正式站沒換。詳細：`docs/audits/COPY-CHAOCHAO-SALES-OPT-2026-09-24.md`。

### 差異 51：潮巢導購寫手拆層（已撤回）

- **差在哪**：曾把寫手核心放到 prompt 最後、3～5 當主軸非上限、看圖可寫具體顏色材質、搜尋 `adv8feat1`。版面仍是潮巢導購五段。
- **為什麼**：Owner 要舊文創意＋看圖顏色材質；CC-5／CC-6 實測幾乎沒差。
- **誰拍板**：Owner 2026-09-23 先授權實作；同日實測 Preview `mrdilc17v` 後說不喜歡，要求回到上一版。
- **現況**：❌ 已 revert。現役是差異 50／CC-6 `14f27c4`／Preview `2u2u3lg60`。不要再當現役實作。

### 差異 50：潮巢導購用 Owner 樣板當正面聲音，規則只留必要作業

- **差在哪**：
  1. 潮巢導購版不再套其他語氣那包長規則。改走專屬短 prompt，開頭放 Owner 認可的雨衣／衝浪／滑雪吊飾寫法。
  2. 正文在四段之後加上「購買提醒」（依材質照顧方式）。舊四段、舊三段稿仍可預覽。
  3. 聲音用正面樣板帶，不加空泛電商句的禁詞池。
- **為什麼**：Owner 2026-09-23 實測覺得上一包幾乎沒差；指定這幾段為要的人味與小幽默，並要求整理規則、改用正面引導。
- **誰拍板**：Owner 2026-09-23。
- **現況**：✅ 現役聲音基準。2026-09-24 在其上做差異 52（具體度／看圖畫面感），沒有改回三段、沒有加禁詞池。Preview 分支 `agent/chaochao-tone-on-live`。尚未部署正式站。

### 差異 49：潮巢導購四段正文＋全站 80 字 IP 在前標題（覆寫 C1.1 三段／品牌在前）

- **差在哪**：
  1. Mockup／C1.1 潮巢正文是「商品介紹、收藏亮點、導購小標」三個 H2；正式潮巢導購版改為「商品介紹、收藏亮點、適合誰、商品資訊」四段。舊三段稿仍可預覽與發布。
  2. 商品標題全語氣改為 `IP中文＋英文 × 品牌 | 精準品項 | 差異`，最多 80 字，不再 80 生成後砍 60；SEO 標題仍是另一欄。
  3. 語氣選擇「潮巢導購版」的說明由「痛點導購」改為「小編導購・具體生活感」。沿用既有 `h2/p/ul/li` 與主題 tokens，不改 globals.css。
- **為什麼**：老闆 2026-09-23 要商品小編感、欄位分工、標題清楚辨識；不要痛點開頭與品牌宣言。
- **誰拍板**：Owner 2026-09-23（本機實作授權；無 push／deploy）。
- **現況**：✅ 已實作於 `agent/chaochao-tone-on-live`，推 Preview 給 Owner 實測。尚未部署正式站、尚未真實 Shopify write。

### 差異 48：擷取預設海外代購；規格不收出貨預售、不留簡繁重複

- **差在哪**：
  1. 外掛新建立的草稿，銷售狀態預設「海外代購（約14天）」，不讀頁面上的預售／現貨。已選過的不會被再抓蓋掉。
  2. 商品規格拿掉出售狀態、預售、發貨、出貨、到貨、庫存、運費這類。產地、包裝、材質、貨號留著。
  3. 同一句簡體和繁體只留一行，並轉成台灣繁體。款式名稱維持頁面原文。
- **為什麼**：老闆 2026-09-22——出貨狀態純人工，不跟淘寶商家；規格不要出貨話術，也不要簡繁各一行。
- **誰拍板**：老闆 2026-09-22。
- **現況**：✅ 已寫進擷取對應（`mapCaptureFields.ts`）。已抓過、規格有字的草稿不會自動改寫。

### 差異 47：電腦版登入置中＋結果列全選／篩選幾何（覆寫 D3.5 checkbox）

- **差在哪**：
  1. 登入頁桌機不再被 `.shell` 兩欄 grid 擠在左側；`.shell--login` 單欄，表單置中（仍沿用 D3.5 `max-width: 640px`）。
  2. 電腦版結果面板標題列只留一顆滑動「全選」（與手機同語意，不是 native checkbox）；標題列下方那顆全選與長按提示列隱藏。
  3. 「▶ 逐件審核／標圖」縮成與左欄「一般｜二手」同高（26px），讓左右面板標題底線平行。
  4. 「只看我的」放在「文案待審核」站 pill **前面**；站 pills／只看我的／排序同高、同 `var(--frame-w)` 框、同 999 圓角。
  5. **只改 `>=960px`**；手機 `<=959` 的 D3.6 滑動全選、三欄篩選、手勢提示不改。
- **為什麼**：老闆 2026-09-11 截圖——登入比例壞掉、全選重複、勾選取代滑動、兩條標題線不齊、只看我的應在站按鈕前面。
- **誰拍板**：老闆 2026-09-11。
- **現況**：✅ D3.11 已實作（`d311-desktop-results-toolbar.css`）；尚未 Preview 給老闆點收。
...
- **誰拍板**：老闆＋X 臨時代理指揮（Grok）2026-07-21。
- **現況**：✅ **UX-B4-P03 已實作＋指揮核帳通過**（2026-07-21｜`6af3a25`）。`tryAutoExpandFromDimensions` 於加／刪軸值後觸發；discard 仍二次確認；類型 `.vh-dim-type` 分層；列「複製」。
...
- 2026-09-23：差異 51 **潮巢導購寫手拆層**——Owner 實測不喜歡，已 revert 回差異 50。
- 2026-09-23：差異 50 **潮巢導購 Owner 樣板＋短規則＋購買提醒**。Preview `14f27c4`／`2u2u3lg60`；現役。
- 2026-09-23（本機）：差異 49 **潮巢導購四段正文＋全站商品標題 IP 在前、最多 80 字**——覆寫 C1.1 三段／品牌在前；舊三段稿相容；無 push／deploy。
...
## 2026-09-24｜商品標題品牌順序（Owner 定案，預覽已退回）

- 差異：Owner 要的標題是「品牌 × IP中文＋英文 | 角色＋商品種類 | 造型或款式差異」。查到可靠英文品牌就用英文。驗收例句：`MINISO × 七龍珠 DRAGON BALL Z | 孫悟空 盲盒擺件 | Q版萌粒鍵帽`。
- 原因：Owner 2026-09-24 指定。程式曾改過，預覽已退回 CC-6（`368e1dc`），這條還沒留在現役程式裡。
- 拍板：等 Codex 確認後再實作。既有草稿不批次改寫。UI 結構與樣式沿用現況。
