...
- **現況**：✅ **已對齊（2026-07-11）**。`ImageUploader.tsx` 收成主圖／詳情圖兩框；
  表單新增「商品規格（手填）」常駐欄位寫 `spec_text`；`/api/analyze-images` 廢除規格
  OCR 路徑、且**不再回寫 `spec_text`**（尊重手填、修掉會被 null 蓋掉的地雷）；詳情圖
  Vision prompt 補上【圖上文字】轉錄段。辨識引擎 `ocrSpecImages` 保留在 visionProvider
  給 **B3** 截圖辨識沿用。待老闆做實機驗收（主圖/詳情圖辨識＋詳情圖廣告字轉錄＋手填規格進文案）。
