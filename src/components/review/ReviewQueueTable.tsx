...
export type ReviewQueueRow = Pick<
  ProductDraft,
  "id" | "title_zh" | "taobao_title" | "status" | "warnings" | "publish_mode"
>;

const PENDING_STATUSES = new Set(["ready_for_review", "needs_revision"]);
const APPROVED_STATUSES = new Set(["approved", "csv_ready"]);
const FAILED_STATUSES = new Set(["api_failed"]);

export function ReviewQueueTable({ drafts }: { drafts: ReviewQueueRow[] }) {
  ...
  function toggleAll() {
    setSelectedIds((current) => (current.size === drafts.length ? new Set() : new Set(drafts.map((draft) => draft.id))));
  }
  ...
  async function batchPublish(mode: "draft" | "active") {
    ...
    const payload = await response.json();
    setBusy(false);
    setMessage(response.ok ? `成功 ${payload.succeeded} 筆／失敗 ${payload.failed} 筆` : payload.error ?? "批次發布失敗");
    setSelectedIds(new Set());
    router.refresh();
  }
