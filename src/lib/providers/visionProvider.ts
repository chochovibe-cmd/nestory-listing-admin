...
【外觀描述】描述你在照片中實際看到的，讓文案寫手能想像畫面：
- 顏色、材質觸感（絨毛／壓克力／塑膠／金屬等視覺可辨識的材質）
- 造型特徵、細節工藝（縫線、印花、配色分布等）
- 畫面感與質感線索（例如絨毛蓬鬆到看起來想抱、雨衣黃在灰背景裡很跳）
- 包裝或配件（如果照片中看得到）
...
規則：
- 外觀描述只寫實際看到的事實。不要行銷語氣（不要出現賣點式誇飾如「超讚」「必入手」），但可以描述畫面感、質感、使用情境的視覺線索（例如「絨毛蓬鬆到會讓人想馬上抱起來」這種偏視覺觀察、非行銷用詞）
...
- 純文字，兩段各用上面的【標題】分隔，總長約 300-500 字；圖上文字很多時優先保留圖上文字，不要為了壓字數刪規格。不要 Markdown
