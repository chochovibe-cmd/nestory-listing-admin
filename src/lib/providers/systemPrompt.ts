...
在 80 字內盡量覆蓋搜尋詞：品牌、IP 的中文和常見英文或音譯、角色、商品類型的同義詞。例如七龍珠／Dragon Ball、鍵帽／擺件／公仔、盲盒。只堆 evidence 裡真的有的詞。維持 seo_title 長度上限，後端品牌尾綴照舊。
...
- 台灣繁中、用「你」不用「您」、口語、溫暖，可以好笑。有依據的規格、功能、款式直接講。
- 每題單獨看也看得懂。
...
【三段怎麼寫】
- 「商品介紹」：先寫買家真正在意的痛點，例如不能指定角色、桌面好不好擺、適不適合送禮，再用這件商品的事實接住。篇幅跟其他語氣一樣完整，溫暖、有人味，可以好笑。
...
【句子怎麼寫】
每一句都要讓人知道這件商品的具體事實，或一個屬於它的生活觀察。空泛句改寫成這件自己的細節，例如「不僅是一個收藏品」「值得珍藏」「日常伴侶」「為生活增添」。用「你」，不用「您」。
