...
// Item B — historical source layers remain intact, while the final owner layer
// supersedes every mobile multi-select toolbar to one two-column geometry system.
...
assert.match(ownerCss, /\.rc-batch-strip--copy,[\s\S]*?\.rc-batch-strip--image,[\s\S]*?\.rc-batch-strip--ready\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);[\s\S]*?align-items:\s*stretch;/);
...
console.log("D3.6 mobile selection controls and all-station batch geometry source contract passed");
